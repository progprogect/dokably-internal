...
export function createTextSvgElementFromSpans(
  ...
  opts: {
    fontSize: number;
    fontFamily: string;
    textAlign: TLDefaultHorizontalAlignStyle | string;
    verticalTextAlign: TLDefaultVerticalAlignStyle | string;
    fontWeight: string;
    fontStyle: string;
    lineHeight: number;
    width: number;
  ...
  textElm.setAttribute('line-height', opts.lineHeight + 'px');
  ...
  for (const { text, box } of spans) {
    ...
    if (didBreakLine) {
      const lineBreakTspan = document.createElementNS(
        'http://www.w3.org/2000/svg',
        'tspan'
      );
  ...
  if (opts.stroke && opts.strokeWidth) {
    ...
    textElm.setAttribute('strokeWidth', opts.strokeWidth + 'px');
  }
