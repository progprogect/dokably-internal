...
const NumberedListPlugin = (
  ...
  return {
    blockRendererFn: (
      ...
      if (contentBlock.getType() === BlockType.NumberedList) {
        return {
    ...
    blockStyleFn: (contentBlock: ContentBlock) => {
      ...
      if (type === BlockType.NumberedList) {
        return 'dokably-numbered-list-block';
      }
    },
    keyBindingFn: (event: KeyboardEvent, pluginFunctions: PluginFunctions) => {
      ...
      if (contentBlock && contentBlock.getType() === BlockType.NumberedList) {
        if (
          event.key === 'Backspace' &&
          !event.shiftKey &&
          !event.altKey &&
...
export default NumberedListPlugin;
