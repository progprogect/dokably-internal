import { ContentBlock, EditorState, convertToRaw } from 'draft-js';
...
const blockType = BlockType.TableOfContent;
...
const TableOfContentPlugin = (
  ...
  return {
    blockRendererFn: (
      ...
      Object.keys(
        convertToRaw(
          pluginFunctions.getEditorState().getCurrentContent(),
        ).blocks.map((el) => {
          if (el.type.includes('header')) {
            if (el.type === 'header-one') {
              headersArray.push({ name: el.text, subHeaders: [] });
            } else if (el.type === 'header-two') {
              if (!headersArray.length) {
                ...
                headersArray.at(-1).subHeaders.push({
                  name: el.text,
                  subHeaders: [],
                });
              }
            } else if (el.type === 'header-three') {
              if (!headersArray.length) {
                headersArray.push({
                  name: '',
                  subHeaders: [
              ...
              } else if (!headersArray.at(-1).subHeaders.length) {
                headersArray.at(-1).subHeaders.push({
                  name: '',
                  subHeaders: [
                    {
                ...
                headersArray.at(-1).subHeaders.at(-1).subHeaders.push({
                  name: el.text,
                  subHeaders: [],
                });
              }
    ...
    blockStyleFn: (contentBlock: ContentBlock) => {
      ...
      if (type === blockType) {
        return 'dokably-table-of-content';
      }
    },
