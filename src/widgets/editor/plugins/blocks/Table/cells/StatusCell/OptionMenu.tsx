...
export const OptionMenu = ({ option, value, allOptions, updateOptions, updateValue, onClose }: Props) => {
  ...
  const handleSave = useCallback(() => {
    if (currentOption.id === value) {
      updateValue(null);
    }
    updateOptions(allOptions.map((item) => (item.id === currentOption.id ? currentOption : item)));

    onClose();
  }, [currentOption, value, updateOptions, allOptions, onClose, updateValue]);
