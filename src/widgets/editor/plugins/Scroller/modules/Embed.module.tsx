...
const EmbedModule = ({ setEditorState, callback }: EditorProps & any) => {
  ...
  const handleSelectTool = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    track('document_edit_embeds_selected', { option: 'embed' });
    setEditorState((editorState: EditorState) => {
      editorState = removeAllInlineStyles(editorState);
      editorState = toggleBlockType(editorState, BlockType.Embed);
      return editorState;
    });
    callback();
  };
