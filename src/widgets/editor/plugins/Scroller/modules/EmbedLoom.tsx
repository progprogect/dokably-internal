...
import { ReactComponent as Embed } from '@images/loom.svg';
...
const EmbedLoomModule = ({ setEditorState, callback }: EditorProps & any) => {
  ...
  const handleSelectTool = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    track('document_edit_embeds_selected', { option: 'loom' });
    setEditorState((editorState: EditorState) => {
      editorState = removeAllInlineStyles(editorState);
      editorState = toggleBlockType(editorState, BlockType.EmbedLoom);
      return editorState;
    });
    callback();
  };
...
export default EmbedLoomModule;
