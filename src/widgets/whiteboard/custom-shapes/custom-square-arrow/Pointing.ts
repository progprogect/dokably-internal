...
export class Pointing extends StateNode {
	...
	createArrowShape() {
		...
		this.editor.createShapes<ICustomSquareArrow>([
			{
				id,
				type: CUSTOM_SQUARE_ARROW_SHAPE_ID,
				x: originPagePoint.x,
				y: originPagePoint.y,
			},
		])

		const shape = this.editor.getShape<ICustomSquareArrow>(id)
		if (!shape) throw Error(`expected shape`)

		const handles = this.editor.getShapeHandles(shape)
		if (!handles) throw Error(`expected handles for arrow`)

		const util = this.editor.getShapeUtil<ICustomSquareArrow>(CUSTOM_SQUARE_ARROW_SHAPE_ID)
