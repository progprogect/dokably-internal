...
export class Pointing extends StateNode {
	...
	createArrowShape() {
		...
		this.editor.createShapes<ICustomSoftArrow>([
			{
				id,
				type: CUSTOM_SOFT_ARROW_SHAPE_ID,
				x: originPagePoint.x,
				y: originPagePoint.y,
			},
		])

		const shape = this.editor.getShape<ICustomSoftArrow>(id)
		if (!shape) throw Error(`expected shape`)

		const handles = this.editor.getShapeHandles(shape)
		if (!handles) throw Error(`expected handles for arrow`)

		const util = this.editor.getShapeUtil<ICustomSoftArrow>(CUSTOM_SOFT_ARROW_SHAPE_ID)
