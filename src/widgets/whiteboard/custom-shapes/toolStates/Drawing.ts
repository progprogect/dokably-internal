...
import { ICustomPen } from '../custom-pen/custom-pen-types';
import { ICustomHighlight } from '../custom-highlight/custom-highlight-types';
import { CUSTOM_DRAW_SHAPE_ID, CUSTOM_HIGHLIGHT_SHAPE_ID } from '@app/constants/whiteboard/shape-ids';

type DrawableShape = ICustomPen | ICustomHighlight;
...
export class Drawing extends StateNode {
  ...
  override shapeType =
    this.parent.id === CUSTOM_HIGHLIGHT_SHAPE_ID
  ...
  canClose() {
    return this.shapeType !== CUSTOM_HIGHLIGHT_SHAPE_ID;
  }

  getIsClosed(segments: TLDrawShapeSegment[], size: number) {
  ...
  private startShape() {
    ...
    if (this.initialShape) {
      ...
      if (shape && this.segmentMode === 'straight') {
        ...
        if (this.canClose()) {
          (shapePartial as TLShapePartial<ICustomPen>).props!.isClosed =
            //@ts-ignore
            this.getIsClosed(segments, shape.props.size);
        }

        this.editor.updateShapes<ICustomPen | ICustomHighlight>([shapePartial]);
      ...
      case 'starting_straight': {
        ...
        if (hasMovedFarEnough) {
          ...
          if (this.canClose()) {
            (shapePartial as TLShapePartial<ICustomPen>).props!.isClosed =
              //@ts-ignore
              this.getIsClosed(segments, size);
          }

          this.editor.updateShapes<DrawableShape>([shapePartial], {
            squashing: true,
          });
        }
        break;
      ...
      case 'starting_free': {
        ...
        if (hasMovedFarEnough) {
          ...
          if (this.canClose()) {
            (shapePartial as TLShapePartial<ICustomPen>).props!.isClosed =
              //@ts-ignore
              this.getIsClosed(finalSegments, size);
          }
