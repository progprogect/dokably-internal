...
import { customRectangleProps } from './custom-rectangle-props';
...
export class CustomRectangleUtil extends BaseBoxShapeUtil<ICustomRectangle> {
  static override type = CUSTOM_RECTANGLE_SHAPE_ID;
  static override props = customRectangleProps;

  override isAspectRatioLocked = (_shape: ICustomRectangle) => false;
  override canResize = (_shape: ICustomRectangle) => true;
  override canBind = (_shape: ICustomRectangle) => true;
  override canEdit = () => true;

  getDefaultProps(): ICustomRectangle['props'] {
    return {
  ...
  getGeometry(shape: ICustomRectangle): Geometry2d {
  ...
  getBounds(shape: ICustomRectangle) {
    return new Box(0, 0, shape.props.w, shape.props.h);
  }

  getCenter(shape: ICustomRectangle) {
    return new Vec(shape.props.w / 2, shape.props.h / 2);
  }

  component(shape: ICustomRectangle) {
    const { id } = shape;
    ...
    useTextMesurements({
      shape: CUSTOM_RECTANGLE_SHAPE_ID,
  ...
  indicator(shape: ICustomRectangle) {
    return <rect width={shape.props.w} height={shape.props.h} />;
  }
  // Events
  override onResize: TLOnResizeHandler<ICustomRectangle> = (shape, info) => {
    return resizeBox(shape, info);
  };

  onResizeEnd: TLOnResizeEndHandler<ICustomRectangle> = (shape) => {
    this.editor.setEditingShape(shape.id);
    const quill = getQuillForShape(shape.id);

    if (!quill) return;

    if (this.editor.getEditingShape()?.id === shape.id) {
      quill.enable();
      quill.focus();
    }
  };

  onEditEnd: TLOnEditEndHandler<ICustomRectangle> = (shape) => {
    const {
      id,
      type,
      props: { text },
