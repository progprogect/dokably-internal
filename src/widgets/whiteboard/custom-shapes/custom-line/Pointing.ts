import { CUSTOM_LINE_SHAPE_ID, CUSTOM_LINE_SHAPE_ID_TYPE } from './../../../../app/constants/whiteboard/shape-ids';
import {
	Mat,
	StateNode,
	TLEventHandlers,
...
export class Pointing extends StateNode {
	...
	shape = {} as ICustomLine

	markId: string | undefined

	override onEnter = (info: { shapeId?: CUSTOM_LINE_SHAPE_ID_TYPE }) => {
		...
		const shape = info.shapeId && this.editor.getShape<ICustomLine>(info.shapeId as TLParentId)
		...
		if (shape && inputs.shiftKey) {
			...
			if (
				...
				nextEndHandle = {
					id: nextEndHandleId,
					type: 'vertex',
					index: nextEndHandleIndex as IndexKey,
					x: nextPoint.x + 0.1,
					y: nextPoint.y + 0.1,
					canBind: false,
				}
			...
			const nextHandles = structuredClone(this.shape.props.points)
			...
			this.editor.createShapes<ICustomLine>([
				{
					id,
	...
	override onPointerMove: TLEventHandlers['onPointerMove'] = () => {
		...
		if (this.editor.inputs.isDragging) {
			...
			this.editor.setCurrentTool('select.dragging_handle', {
				...
				onInteractionEnd: CUSTOM_LINE_SHAPE_ID,
			})
		}
	}
