...
export class Pointing extends StateNode {
	...
	createArrowShape() {
		...
		this.editor.createShapes<ICustomStraightArrow>([
			{
				id,
				type: CUSTOM_STRAIGHT_ARROW_SHAPE_ID,
				x: originPagePoint.x,
				y: originPagePoint.y,
			},
		])

		const shape = this.editor.getShape<ICustomStraightArrow>(id)
		if (!shape) throw Error(`expected shape`)

		const handles = this.editor.getShapeHandles(shape)
		if (!handles) throw Error(`expected handles for arrow`)

		const util = this.editor.getShapeUtil<ICustomStraightArrow>(CUSTOM_STRAIGHT_ARROW_SHAPE_ID)
