...
import { DocablyArrowColor, DokablyEndAnchorPosition, DokablyStartAnchorPosition } from '@app/constants/whiteboard/whiteboard-styles'
import { CUSTOM_STRAIGHT_ARROW_SHAPE_ID } from '@app/constants/whiteboard/shape-ids'
import { customStraightArrowShapeProps } from './custom-straight-arrow-props'
import { ARROW_DEFAULT_SIZE } from '@app/constants/whiteboard/constants'
import { getCurvedArrowHandlePath, getSolidCurvedArrowPath, getSolidStraightArrowPath, getStraightArrowHandlePath } from '@widgets/whiteboard/utils'
import { getArrowheadPathForType } from '@widgets/whiteboard/arrowheads'
import { BoundShapeInfo, mapObjectMapValues, objectMapEntries } from '@widgets/whiteboard/arrows-common-utils'
...
export class CustomStraightArrowUtil extends ShapeUtil<ICustomStraightArrow> {
  static override type = CUSTOM_STRAIGHT_ARROW_SHAPE_ID;
  static override props = customStraightArrowShapeProps;

  override canEdit = () => true;
	override canBind = () => false;
  override canSnap = () => false;
  override hideResizeHandles: TLShapeUtilFlag<ICustomStraightArrow> = () =>
    true;
  override hideRotateHandle: TLShapeUtilFlag<ICustomStraightArrow> = () => true;
  override hideSelectionBoundsBg: TLShapeUtilFlag<ICustomStraightArrow> = () =>
    true;
  override hideSelectionBoundsFg: TLShapeUtilFlag<ICustomStraightArrow> = () =>
    true;

  override getDefaultProps(): ICustomStraightArrow['props'] {
    return {
      dash: 'draw',
  ...
  getGeometry(shape: ICustomStraightArrow) {
  ...
  override getHandles(shape: ICustomStraightArrow): TLHandle[] {
    const info = getStraightArrowInfo(this.editor, shape);
    return [
      {
        id: 'start',
	...
	override onHandleDrag: TLOnHandleDragHandler<ICustomStraightArrow> = (shape, { handle, isPrecise }) => {
		...
		const next = structuredClone(shape) as ICustomStraightArrow
  ...
  override onTranslateStart: TLOnTranslateStartHandler<ICustomStraightArrow> = (shape) => {
  ...
  override onTranslate?: TLOnTranslateHandler<ICustomStraightArrow> = (initialShape, shape) => {
  ...
  override onResize: TLOnResizeHandler<ICustomStraightArrow> = (
    shape,
    info
  ) => {
  ...
  override onDoubleClickHandle = (
    shape: ICustomStraightArrow,
    handle: TLHandle
  ): TLShapePartial<ICustomStraightArrow> | void => {
    switch (handle.id) {
      case 'start': {
        return {
          id: shape.id,
  ...
  component(shape: ICustomStraightArrow) {
  ...
  indicator(shape: ICustomStraightArrow) {
...
function getStraightArrowInfo(
  ...
  if (!isSelfIntersection) {
    ...
    if (
      ...
      const endShape = endShapeInfo.shape as ICustomStraightArrow;
...
function updateArrowheadPointWithBoundShape(
  ...
  if (intersection !== null) {
    targetInt =
      intersection.sort(
        (p1, p2) => Vec.Dist(p1, targetFrom) - Vec.Dist(p2, targetFrom)
  ...
  const arrowInt = Mat.applyToPoint(
