...
import { MindMapSoftArrowUtil } from './util';

export class Pointing extends StateNode {
	static override id = 'pointing'

	shape?: IMindMapSoftArrow
	...
	createArrowShape() {
		...
		this.editor.createShapes<IMindMapSoftArrow>([
			{
				id,
				type: MIND_MAP_SOFT_ARROW_SHAPE_ID,
				x: originPagePoint.x,
				y: originPagePoint.y,
			},
		])

		const shape = this.editor.getShape<IMindMapSoftArrow>(id)
		if (!shape) throw Error(`expected shape`)

		const handles = this.editor.getShapeHandles(shape)
		if (!handles) throw Error(`expected handles for arrow`)

		const util = this.editor.getShapeUtil<IMindMapSoftArrow>(MIND_MAP_SOFT_ARROW_SHAPE_ID)
		const initial = this.shape
		const startHandle = handles.find((h) => h.id === 'start')!
		const change = (util as MindMapSoftArrowUtil).onHandleDrag(shape, {
	...
	updateArrowShapeEndHandle() {
		...
		// end update
		{
			...
			const change = (util as MindMapSoftArrowUtil).onHandleDrag(shapeWithOutEndOffset, {
