...
const HoverMindMapAddChildBtns: React.FC = () => {
  ...
  const showBtns = useMemo(() => {
    ...
    ).includes(hoveredShape.type);
  }, [hoveredShape]);

  const buttonsToShow = useMemo(() => {
    if (!showBtns && !hoveredShape) return [];

    if (hoveredShape?.type === MIND_MAP_MAIN_INPUT_SHAPE_ID) {
      switch ((hoveredShape as IMindMapMainInput).props.side) {
        case 'bottom':
        case 'top':
          return ['top', 'bottom'];
        case 'left':
    ...
    if (hoveredShape?.type === MIND_MAP_CHILD_INPUT_SHAPE_ID) {
      switch ((hoveredShape as IMindMapChildInput).props.side) {
        case 'bottom':
          return ['bottom'];
        case 'top':
          return ['top'];
    ...
    return [];
  }, [hoveredShape, showBtns]);

  useLayoutEffect(() => {
    if (!hoveredShape) return;

    const getPageBounds = editor.getShapePageBounds(hoveredShape) as Box;
    ...
    const calculateTopBtnStyle = () => {
      ...
      const left =
        (x + getPageBounds?.center.x) * z - 20 - ADD_CHILD_BTN_WIDTH / 2;

      const abovePositionY = (y + getPageBounds.minY) * z - 46;
      const belowPositionY = (y + getPageBounds.maxY) * z - 10;
    ...
    const calculateRightBtnStyle = () => {
      ...
      const { x, y, z } = editor.getCamera();
      const left = (x + getPageBounds.maxX) * z - 10;
      const top = (y + getPageBounds.center.y) * z - 10;
    ...
    const calculateLeftBtnStyle = () => {
      ...
      const { x, y, z } = editor.getCamera();
      const left = (x + getPageBounds.minX) * z - ADD_CHILD_BTN_WIDTH - 30;
      const top = (y + getPageBounds.center.y) * z - 10;
  ...
  const calculateInputWithChildrenHeight = (
    inputShapeId: MIND_MAP_CHILD_INPUT_SHAPE_ID_TYPE
  ): number => {
    const input = editor.getShape(
      inputShapeId as TLShapeId
    ...
    const childrenInputsHeights = childrenInputs.map((shape) =>
      calculateInputWithChildrenHeight(
        shape.id as MIND_MAP_CHILD_INPUT_SHAPE_ID_TYPE
      )
    );
    ...
    const resultHeight =
      input.props.h > heightOfAllChildren ? input.props.h : heightOfAllChildren;

    return resultHeight;
  };

  const getHeightWithChildren = (
    shape: IMindMapMainInput | IMindMapChildInput
  ): number => {
    ...
    const childrenInputs = children.filter(
      (item) =>
        ...
        item?.props?.side === shape.props.side
    );
  ...
  const createRightChildToMain = useCallback(
    (side: Side) => {
      const currentShape = hoveredShape as IMindMapChildInput;
  ...
  const createRightChildToChild = useCallback(
    (side: Side) => {
      const currentShape = hoveredShape as
        | IMindMapChildInput
        | IMindMapMainInput;
      const existingRightSideInputs = getChildrenInputs(currentShape, side);
      ...
      const moveAllInputsDueToMaxHChange = () => {
        setTimeout(() => {
          const parentInput = editor.getShapeParent(
            currentShape.id
          ) as IMindMapChildInput;
          ...
          const allChildrenInputsHeight =
            childrenInputs.reduce((acc, currVal) => {
              return acc + currVal.props.heightWithChildren + 20;
            }, 0) - 20;

          let highestPoint =
            parentInput.props.h / 2 - allChildrenInputsHeight / 2;
  ...
  const createRightChildInput = useCallback(
    (side: Side) => {
      const currentShape = hoveredShape as
        | IMindMapChildInput
        | IMindMapMainInput;
      if (currentShape.type === MIND_MAP_MAIN_INPUT_SHAPE_ID)
        return createRightChildToMain(side);
      if (currentShape.type === MIND_MAP_CHILD_INPUT_SHAPE_ID)
        return createRightChildToChild(side);
    },
    [hoveredShape, editor]
  );
  ...
  const createLeftChildToMain = useCallback(
    (side: Side) => {
      ...
      const newInputId = createShapeId();
      const newInputX =
        -HORIZONTAL_GAP_BTW_INPUTS - MIND_MAP_CHILD_INPUT_DEFAULT_WIDTH;
  ...
  const createLeftChildInput = useCallback(
    (side: Side) => {
      const currentShape = hoveredShape as
        ...
        | IMindMapMainInput;
      if (currentShape.type === MIND_MAP_MAIN_INPUT_SHAPE_ID)
        return createLeftChildToMain(side);
      if (currentShape.type === MIND_MAP_CHILD_INPUT_SHAPE_ID)
        return createLeftChildToChild(side);
    },
    [hoveredShape, editor]
  );
    ...
    (side: Side) => {
      ...
      const moveAllInputsDueToMaxWidthChange = () => {
        setTimeout(() => {
          ...
          let mostLeftPoint =
            parentInput.props.w / 2 - allChildrenInputsWidth / 2;
  ...
  const createTopChildInput = useCallback(
    (side: Side) => {
      const currentShape = hoveredShape as
        ...
        | IMindMapMainInput;
      if (currentShape.type === MIND_MAP_MAIN_INPUT_SHAPE_ID)
        return createTopChildToMain(side);
      if (currentShape.type === MIND_MAP_CHILD_INPUT_SHAPE_ID)
        return createTopChildToChild(side);
    },
    [hoveredShape, editor]
  );
  ...
  const createBottomChildInput = useCallback(
    (side: Side) => {
      const currentShape = hoveredShape as
        ...
        | IMindMapMainInput;
      if (currentShape.type === MIND_MAP_MAIN_INPUT_SHAPE_ID)
        return createBottomChildToMain(side);
      if (currentShape.type === MIND_MAP_CHILD_INPUT_SHAPE_ID)
        return createBottomChildToChild(side);
    },
    [hoveredShape, editor]
  );
  ...
  return (
    <div className='tl-html-layer' ref={rHtmlLayer}>
      ...
      {buttonsToShow.includes('right') && (
        <div
          className={styles.addChildInputBtn}
          onPointerDown={() => createChildInput('right')}
          style={{
            ...commonBtnStyle,
            ...rightBtnStyle,
          }}
      ...
      {buttonsToShow.includes('bottom') && (
        <div
          className={styles.addChildInputBtn}
          onPointerDown={() => createChildInput('bottom')}
          style={{
            ...commonBtnStyle,
            ...bottomBtnStyle,
          }}
...
export default HoverMindMapAddChildBtns;
