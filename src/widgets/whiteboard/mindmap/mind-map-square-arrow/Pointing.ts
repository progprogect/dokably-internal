...
import { StateNode, TLEventHandlers, createShapeId } from '@tldraw/editor'
import { MindMapSquareArrowUtil } from './util';
...
export class Pointing extends StateNode {
	...
	createArrowShape() {
		...
		this.editor.createShapes<IMindMapSquareArrow>([
			{
				id,
				type: MIND_MAP_SQUARE_ARROW_SHAPE_ID,
				x: originPagePoint.x,
				y: originPagePoint.y,
			},
		])

		const shape = this.editor.getShape<IMindMapSquareArrow>(id)
		if (!shape) throw Error(`expected shape`)

		const handles = this.editor.getShapeHandles(shape)
		if (!handles) throw Error(`expected handles for arrow`)

		const util = this.editor.getShapeUtil<IMindMapSquareArrow>(MIND_MAP_SQUARE_ARROW_SHAPE_ID)
		const initial = this.shape
		const startHandle = handles.find((h) => h.id === 'start')!
		const change = (util as MindMapSquareArrowUtil).onHandleDrag(shape, {
	...
	updateArrowShapeEndHandle() {
		...
		// end update
		{
			...
			const change = (util as MindMapSquareArrowUtil).onHandleDrag(shapeWithOutEndOffset, {
