...
import { useReadonly } from '@tldraw/tldraw';

import {
  MIND_MAP_CHILD_INPUT_SHAPE_ID,
  MIND_MAP_MAIN_INPUT_SHAPE_ID,
} from '@app/constants/whiteboard/shape-ids';
import { ICustomText } from '@widgets/whiteboard/custom-shapes/custom-text/custom-text-types';

/** @public */
export const CustomSelectionForeground = track(
  function TldrawSelectionForeground({
    bounds,
    rotation,
  }: TLSelectionForegroundProps) {
    ...
    const isDefaultCursor =
      ...
      editor.getInstanceState().cursor.type === 'default';
    const isCoarsePointer = editor.getInstanceState().isCoarsePointer;

    const onlyShape = editor.getOnlySelectedShape();

    const isLockedShape =
      onlyShape && editor.isShapeOrAncestorLocked(onlyShape);

    // if all shapes have an expandBy for the selection outline, we can expand by the l
    const expandOutlineBy = onlyShape
      ? (editor.getShapeUtil(onlyShape) as any)?.expandSelectionOutlinePx(
          onlyShape
        )
      : 0;
    ...
    const targetSizeX =
      (isSmallX ? targetSize / 2 : targetSize) *
      (mobileHandleMultiplier * 0.75);
    const targetSizeY =
      (isSmallY ? targetSize / 2 : targetSize) *
      (mobileHandleMultiplier * 0.75);

    const showSelectionBounds =
      (onlyShape
        ? !editor.getShapeUtil(onlyShape).hideSelectionBoundsFg(onlyShape)
        : true) && !isChangingStyle;
    ...
    let shouldDisplayBox =
      ...
      (showSelectionBounds &&
        ...
        onlyShape &&
        editor.isShapeOfType<ICustomText>(onlyShape, 'custom-text'));
    ...
    let hideRotateCornerHandles = !showCornerRotateHandles;
    const hideMobileRotateHandle =
      !shouldDisplayControls || !showMobileRotateHandle;
    let hideTopLeftCorner = !shouldDisplayControls || !showHandles;
    let hideTopRightCorner =
      !shouldDisplayControls || !showHandles || hideAlternateCornerHandles;
    let hideBottomLeftCorner =
      !shouldDisplayControls || !showHandles || hideAlternateCornerHandles;
    let hideBottomRightCorner =
      !shouldDisplayControls ||
      !showHandles ||
    ...
    if (showCropHandles) {
      ...
      hideVerticalEdgeTargets =
        hideAlternateCornerHandles || showOnlyOneHandle || isCoarsePointer;
      const isMobileAndTextShape =
        isCoarsePointer && onlyShape && onlyShape.type === 'text';
      hideHorizontalEdgeTargets =
    ...
    if (
      onlyShape &&
      (onlyShape.type === MIND_MAP_MAIN_INPUT_SHAPE_ID ||
        onlyShape.type === MIND_MAP_CHILD_INPUT_SHAPE_ID)
    ) {
      hideVerticalEdgeTargets = true;
      hideTopLeftCorner = true;
      hideTopRightCorner = true;
      hideBottomLeftCorner = true;
      hideBottomRightCorner = true;
      hideRotateCornerHandles = true;
    }
      ...
      >
        <g ref={rSvg}>
          ...
          <RotateCornerHandle
            ...
            cursor={
              isDefaultCursor ? getCursor('nwse-rotate', rotation) : undefined
            }
          ...
          <RotateCornerHandle
            ...
            cursor={
              isDefaultCursor ? getCursor('nesw-rotate', rotation) : undefined
            }
            isHidden={hideRotateCornerHandles}
          />
          <RotateCornerHandle
            ...
            cursor={
              isDefaultCursor ? getCursor('swne-rotate', rotation) : undefined
            }
            isHidden={hideRotateCornerHandles}
          />
          <RotateCornerHandle
            ...
            cursor={
              isDefaultCursor ? getCursor('senw-rotate', rotation) : undefined
            }
            isHidden={hideRotateCornerHandles}
          />
          <MobileRotateHandle
          ...
          <rect
            ...
            pointerEvents='all'
            x={toDomPrecision(
              0 - (isSmallX ? targetSizeX * 2 : targetSizeX * 1.5)
            )}
            y={toDomPrecision(
              0 - (isSmallY ? targetSizeY * 2 : targetSizeY * 1.5)
            )}
          ...
          <rect
            ...
            pointerEvents='all'
            x={toDomPrecision(
              0 - (isSmallX ? targetSizeX * 3 : targetSizeX * 1.5)
            )}
